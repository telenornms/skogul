...
func TestSNMPConnectFailureIsNotPermanent(t *testing.T) {
	x := testSNMP(162)
	// This test is about the socket being redialled after a failure,
	// not the holdoff - turn that off so the second Send is attempted
	// rather than held off after the first one's failures.
	disabled := false
	x.BackoffEnabled = &disabled
...
// the rest of the container going, but a container where *every* metric
// is dropped is a configuration mismatch and must fail synchronously so
// the caller learns nothing was delivered.
func TestSNMPSendSkipsEmptyTraps(t *testing.T) {
	...
	if err := x.Send(&empty); err == nil {
		t.Errorf("a container where no metric could be mapped should error")
	}

	if err := x.Send(&skogul.Container{}); err != nil {
		t.Errorf("a container with no metrics should not error, got: %v", err)
	}
}
