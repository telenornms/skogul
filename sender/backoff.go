...
func (bo *Backoff) Deprecated() error {
	return fmt.Errorf("the 'backoff' sender wrapper is deprecated and will be removed in a future version - network senders now back off from a failing target on their own (see BackoffEnabled, on by default), and retries are available on the sender itself (see MaxRetries)")
}
