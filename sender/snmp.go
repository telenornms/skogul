...
func (x *SNMP) Send(c *skogul.Container) error {
	...
	if len(traps) == 0 {
		if len(c.Metrics) > 0 {
			// The whole container was dropped, which is a
			// configuration mismatch rather than a network
			// problem: fail synchronously, without retrying.
			return fmt.Errorf("no metrics in container of %d could be mapped to SNMP traps, check Oidmap and SnmpTrapOID", len(c.Metrics))
		}
		return nil
	}
	pending := traps
